...
const FacultyComplaint = mongoose.model("faculty", complaintSchema);
router.put("/status/:id", (req, res) => {
  const { id } = req.params;
  const { status } = req.body;

  FacultyComplaint.findByIdAndUpdate(
    id,
    { status },
    { new: true },
...
router.delete("/delete-complaint/:id", (req, res) => {
  ...
  FacultyComplaint.findByIdAndRemove(id, (err, deletedComplaint) => {
    if (err) {
